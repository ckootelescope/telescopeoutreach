...
select
  ...
  (select count(*) from step s join sequence q on q.id = s.sequence_id
     where s.drafted_at is not null and s.sent_at is null
       and s.due_date < current_date - 2)                                     as drafted_never_sent,
  (select count(*) from (
...
select c.name as company, ct.name as founder, ct.email,
 ...
 where s.status = 'planned' and q.status = 'active' and s.due_date <= current_date
...
select c.name as company, cd.domain, ct.name as founder, ct.email,
       s.sent_at::date as opener_sent,
       current_date - s.sent_at::date as days_since,
       q.kind, q.round
  from sequence q
  join company  c on c.id = q.company_id
  join contact ct on ct.id = q.contact_id
...
select c.name as company, ct.email, q.kind, q.round, q.status,
       ...
       max(s.sent_at)::date      as last_sent
  from sequence q
  join company  c on c.id = q.company_id
  join contact ct on ct.id = q.contact_id
...
select c.name as company, ct.name as founder, ct.email,
       max(e.sent_at) filter (where e.direction = 'in')::date  as last_inbound,
       max(e.sent_at) filter (where e.direction = 'out')::date as last_outbound,
       current_date - max(e.sent_at) filter (where e.direction = 'in')::date as days_waiting
  from email_event e
...
select c.name as company, c.primary_domain, ct.email,
       min(s.sent_at)::date as first_contacted
  from sequence q
  join company  c on c.id = q.company_id
  join contact ct on ct.id = q.contact_id
  join step     s on s.sequence_id = q.id
