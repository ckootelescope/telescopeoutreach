...
async function main() {
  const domains = JSON.parse(fs.readFileSync('_breakin_domains.json', 'utf-8'));
  const token = await getAccessToken();

  const values = domains.map(d => [oneLiners[d.company] || '']);

  const filled = values.filter(v => v[0] !== '').length;
  console.log('Filled: ' + filled + '/' + domains.length);

  const range = "'Monthly Break-In'!E3:E" + (3 + domains.length - 1);
  const result = await updateValues(token, range, values);
  console.log('Written: ' + (result.updatedCells || 0) + ' cells');
  console.log('Range: ' + (result.updatedRange || ''));

  const missing = domains.filter(d => !oneLiners[d.company]).map(d => d.company);
  if (missing.length > 0) {
    console.log('Missing one-liners for: ' + missing.join(', '));
  }
}
